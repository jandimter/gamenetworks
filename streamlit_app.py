...
def build_student_feedback(game, student_id):
    game.compute_ranking()
    rankings = build_rankings(game)
    student_row = next((row for row in rankings if row["User"] == student_id), None)

    if student_row is None:
        return []

    feedback = [
        f"Tu posición en **Indegree** es **#{student_row['Rank Indegree']}**.",
        f"Tu posición en **Clustering** es **#{student_row['Rank Clustering']}**.",
        f"Tu posición en **Betweenness** es **#{student_row['Rank Betweenness']}**.",
    ]
    ...
    for metric in ["Indegree", "Clustering", "Betweenness"]:
        delta_value = student_row.get(f"Δ {metric}", "—")
        if delta_value.startswith("↑"):
            movement = delta_messages["↑"]
            amount = delta_value.replace("↑", "").strip()
            feedback.append(f"En **{metric}** {movement} **{amount}** puesto(s) respecto a la ronda anterior.")
        elif delta_value.startswith("↓"):
            movement = delta_messages["↓"]
            amount = delta_value.replace("↓", "").strip()
            feedback.append(f"En **{metric}** {movement} **{amount}** puesto(s) respecto a la ronda anterior.")
        elif delta_value.startswith("="):
            feedback.append(f"En **{metric}** {delta_messages['=']} respecto a la ronda anterior.")
        else:
            feedback.append(f"En **{metric}**: {delta_messages['—']}.")

    available_targets = game.unconnectedOF(student_id)
    if available_targets:
        top_targets = sorted(
            available_targets,
            key=lambda node_id: game.graph.nodes[node_id].get("indegree", 0),
            reverse=True,
        )[:3]
        suggested_names = [game.aliases.get(node_id, node_id) for node_id in top_targets]
        feedback.append(
            "Sugerencia para la próxima ronda: conecta con nodos bien posicionados en indegree, por ejemplo: "
...
def Changes_screen():
    ...
    st.subheader("Feedback pedagógico")
    st.caption("Interpretación de tu posición actual y su cambio respecto de la ronda anterior.")
    feedback_lines = build_student_feedback(st.session_state.Game, st.session_state.User.id)
